...
xfree_apk_run_pkg_op() {
  ...
  # Без пайпа: в ash код выхода `apk | tee` — это код tee, и сбойный upgrade
  # выглядит как успех («пакет установлен»).
  if [ "$_mode" = "upgrade" ]; then
    apk upgrade "$APK_PKG_NAME" >"$_tmp_log" 2>&1 || _rc=$?
  else
    apk add "$APK_PKG_NAME" >"$_tmp_log" 2>&1 || _rc=$?
  fi
  if [ -s "$_tmp_log" ]; then
    cat "$_tmp_log" >&2
  fi
...
Options:
  ...
  --apply-mode <mode>   instantiate (default) | sync
                         ...
                         если каталога TEMPLATE_REF уже нет, имя ищется в TEMPLATE_ALIAS другого шаблона;
                         обновление идёт из этого каталога, TEMPLATE_REF переписывается на его имя
